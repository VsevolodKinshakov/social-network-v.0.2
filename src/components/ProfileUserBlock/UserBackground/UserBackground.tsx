...
import { BackgroundImage, DefaultBackground } from "./UserBackground.style";

interface ProfileBackgroundProps {
  backgroundUrl?: string | null;
  onBackgroundChange: (backgroundUrl: string) => void; // Обновленный обработчик
}
...
export const ProfileBackground = ({
  ...
  const handleBackgroundChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event?.target?.files && event?.target?.files[0]) {
      ...
      // Объект FileReader для чтения выбранного файла
      const reader = new FileReader();
      reader.readAsDataURL(file);

      reader.onload = function () {
        // URL выбранного файла и обновляем состояние компонента
        const backgroundUrl = reader.result as string;
        setSelectedBackground(backgroundUrl);

        // Обработчик, чтобы передать URL наружу или на сервер
  ...
  return (
    <>
      {selectedBackground ? (
        <BackgroundImage style={{ backgroundImage: `url(${selectedBackground})` }} />
      ) : (
        <DefaultBackground className="profile-background" />
      )}
      <label htmlFor="background-upload" className="edit-background-button">
        <svg
      ...
      <input
        ...
        type="file"
        onChange={handleBackgroundChange} // Обновленный обработчик
        style={{ display: "none" }}
      />
    </>
  );
